...
def LISTINT3(url):
        ...
        if urllist:
                ...
                match=re.compile('<a href="(.+?)"><img width=".+?" height=".+?" src="(.+?)" alt="(.+?)" title=".+?" /></a>',re.DOTALL).findall(urllist)
                ...
                for url,thumb,name in match:
                        name=name.replace('ver online','').replace('Ver Online','').replace('Ver ','').replace(' Online','')
