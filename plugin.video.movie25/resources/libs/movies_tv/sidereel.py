...
def MAINSIDE(cacheOnly = False):
    ...
    if (not cached or (cached and time.mktime(datetime.date.today().timetuple()) > os.stat(cached_path).st_mtime)
        ...
        today=re.compile("<li class='current'><div class='calendar-day'><h2 class='header calendar-font'><span>(.+?)</span></h2><div class=''(.+?)/div></div></li>",re.DOTALL).findall(link)
        match=re.compile("<li class=''><div class='calendar-day'><h2 class='header calendar-font'><span>(.+?)</span></h2><div class=''(.+?)/div></div></li>",re.DOTALL).findall(link)
        match = today + match
        if match:
    ...
    for date,shows in match:
        if 'data-track-label="Profile">' in shows:
            ...
            match2=re.compile("""<div class='calendar-entry'><a href=".+?" class=".+?".+?data-track-label="Profile">(.+?)</a><div><a href="(.+?)" class=".+?data-track-label="Profile">(.+?)</a></div(.+?)/div>""",re.DOTALL).findall(shows)
